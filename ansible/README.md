...
このリポジトリ全体の説明・他ディレクトリとの関係は、上位の [README.md](../README.md) を参照してください。

**実行場所:** 以下の `ansible-playbook` 例は、リポジトリ内の **`ansible/` をカレントディレクトリにして** 実行することを前提としています（`cd ansible` のうえで `-i inventory ...`）。
...
├── setup-k3s-environment.yml # k3s + CLI（kubectl / k9s / kubectx / kubens）まとめて
├── install-kubectl.yml # kubectl バイナリのみ（inventory の全ホスト）
...
├── install-k9s.yml # localhost に k9s
...
`install-k3s.yml`・`install-k9s.yml` などは、上記の `roles/k3s`・`roles/k9s` などを呼び出す薄い playbook です。既定変数は各 `roles/<名前>/defaults/main.yml` を参照してください。

### k3s とローカル CLI（kind を使わない場合）
...
| `install-kubectl.yml` | 公式バイナリで kubectl を導入（`roles/kubectl`） | `hosts: all`, `become: yes`。アーキテクチャは `ansible_architecture` から amd64/arm64 を選択 |
| `install-k9s.yml` | k9s を **localhost** に導入 | `hosts: localhost`, `become: yes` |
| `install-sops.yml` | [SOPS](https://github.com/getsops/sops) を **localhost** に導入（GitHub Releases の linux amd64/arm64） | `hosts: localhost`, `become: yes`。SOPS + age を使う場合は別途 `age` のインストールが必要 |
...
| `setup-k3s-environment.yml` | k3s（全ホスト）＋ 実行マシンに kubectl / k9s / kubectx / kubens | `--tags k3s` で k3s のみ、`--tags kubectl,k9s,kubectx_tools` で CLI のみ |
...
このリポジトリにコミットされている `inventory` の内容は次のとおりです。**リモートの IP やグループ構成は自分の環境に合わせて編集**してください。
...
`-l` で対象ホストを限定できます（例: `-l 217.142.224.159`）。
