...
- `linux-setup.yml` のデフォルトでは、制御ノードに公開鍵 `~/.ssh/oci_terraform_key.pub` が存在すること
...
├── inventory                    # ホスト定義
├── linux-setup.yml              # ansible / k8s ユーザーと SSH 鍵
...
├── install_k9s.yml              # 制御マシン（localhost）に k9s
├── install_kubectx.yml          # kubectx（connection: local）
├── install_kubens.yml           # kubens（connection: local）
└── roles/                       # 現行 playbook からは未参照（再利用用のタスク集）
    ├── docker/
    └── user_k8s/
```
...
### 1. ユーザー作成と SSH 鍵（`linux-setup.yml`）
...
| `install_k9s.yml` | k9s を **localhost** に導入 | `hosts: localhost`, `become: yes` |
| `install_kubectx.yml` / `install_kubens.yml` | kubectx / kubens を `connection: local` で配置 | インベントリにリモートホストが含まれるとホストごとにプレイが走るため、制御マシンだけに入れたい場合は `-l localhost` などで限定するとよい |
...
1. **実行順序**: kind 流儀では `linux-setup.yml` → `setup-kind.yml` の順が前提です。
2. **アーキテクチャ**: `setup-kind.yml` と `install-helm.yml` の `arch` は対象マシンに合わせてください。
3. **SSH 鍵**: `linux-setup.yml` の `public_key_path` は必要に応じて `-e` で上書きできます。
